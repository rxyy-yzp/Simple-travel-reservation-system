...
namespace 简单旅行预订系统
{
    public partial class AdminViewHotel : Form
    {
        public AdminViewHotel()
        {
            InitializeComponent();
        }
        ...
        public void Table()
        {
            ...
            Dao dao = new Dao();
            string sql = "select * from RESV_HOTELS";
        ...
        private void AdminViewHotel_Load(object sender, EventArgs e)
        {
            Table();
        }
        ...
        public void TableHotelWithN()
        {
            dataGridView1.Rows.Clear();//清空已有数据
            Dao dao = new Dao();
            string sql = $"select * from RESV_HOTELS where resvNum LIKE '%{textBox1.Text}%' and custID LIKE '%{textBox2.Text}%' and  resvHotelName LIKE '%{textBox3.Text}%'";
            IDataReader dc = dao.read(sql);
            while (dc.Read())//循环读取每行数据
            {
                dataGridView1.Rows.Add(dc[0].ToString(), dc[1].ToString(), dc[2].ToString(), dc[3].ToString());
